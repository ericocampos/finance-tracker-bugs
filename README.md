...
| **App version** | `0.19.8` (delivered via OTA on top of the 0.19.4 APK below) |
...
| **OTA tag (current)** | `mobile-v0.19.8-preview` — published 2026-05-07. The OTA arrives automatically on next foreground; **no reinstall needed** if you're already on 0.19.4 or any later 0.19.x OTA. From 0.19.7 onward, the app also shows an in-app prompt when a new OTA has been fetched — see *What's new* below. |
| **How to confirm you're on the latest** | Open the app → Configurações → Sobre → check the version. Should read `0.19.8`. If you still see an earlier `0.19.x` after a foreground/background cycle and a network connection, force-quit and reopen. From 0.19.8 you can also tap the **Versão** row directly to trigger a manual check. |
| **OTA channel** | `preview`, mapped to `runtimeVersion: "1"` (hand-managed, decoupled from app version). Future patch releases on the same runtime arrive as silent OTAs — no APK reinstall needed. A fresh APK is only required when the runtime version itself changes (native module additions or Expo SDK upgrade). |
| **What's new since 0.19.5 (this OTA chain)** | **0.19.8 — Manual "tap to check" on the Versão row.** When no update is pending, the **Sobre / Versão** row reads `Versão 0.19.8 — toque para verificar` (pt) / `tap to check` (en); tapping fires an OTA check. While the check runs, the row briefly shows `a verificar… / verificando… / checking…` (disabled). If no update is found, it briefly reads `já estás na versão mais recente / você está na versão mais recente / you're on the latest` for ~3 s. If the check fails (offline, network error), it briefly reads `não foi possível verificar / couldn't check` for ~3 s in red and remains tappable for retry. **0.19.7 — In-app OTA update prompt.** When an OTA update has been downloaded and is waiting to be applied, the bottom **Settings tab** shows a small accent-coloured notification dot, and the Versão row flips to `… toque para reiniciar / tap to restart`. Tapping the row reloads the app on the new bundle. The auto-check fires on cold start and on app foreground, debounced to once every 30 minutes. **0.19.6 — Insights cumulative-balance fix.** The cumulative-balance line chart on the Insights tab no longer overflows below the card when balances go negative — the y-axis now mirrors symmetrically across zero so the whole line is visible without scrolling. (Closes public bug #7.) See `docs/app-behavior.md` §4.13 and §5.5. |
