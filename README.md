...
| **App version** | `0.19.5` (delivered via OTA on top of the 0.19.4 APK below) |
...
| **OTA tag (current)** | `mobile-v0.19.5-preview` — published 2026-05-05. The OTA arrives automatically on next foreground; **no reinstall needed** if you're already on 0.19.4. |
| **How to confirm you're on the latest** | Open the app → Configurações → Sobre → check the version. Should read `0.19.5`. If you still see `0.19.4` after a foreground/background cycle and a network connection, force-quit and reopen. |
...
| **What's new in 0.19.5 (this OTA)** | **Confirm before deleting a transaction.** Tapping the Delete button on the transaction edit screen now opens a native two-button dialog ("Cancel" / "Delete") and only deletes after explicit confirmation. For transfers, the body line warns that **both legs of the transfer will be deleted** (matched by `transferGroupId` — same behaviour as before, just no longer silent). Localised across en / pt-BR / pt-PT. See `docs/app-behavior.md` §3.4. |
