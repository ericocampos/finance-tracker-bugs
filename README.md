...
| **App version** | `0.19.4` |
| **Install link (REQUIRED — please reinstall)** | https://expo.dev/accounts/ericocampos/projects/finance-tracker-mobile/builds/023b15db-8253-425e-ac18-6a9d59d7996e |
| **Direct APK download** | https://expo.dev/artifacts/eas/gYFixNbX3gSD6nbdu9eLA.apk |
| **APK Build ID** | `023b15db-8253-425e-ac18-6a9d59d7996e` |
| **⚠ Why reinstall is required** | The earlier 0.19.1 APK was built without an EAS Update channel embedded, which meant **no OTA update we shipped (0.19.2 or 0.19.3) ever reached installed devices**. We fixed the build configuration in 0.19.4. After installing this APK once, every subsequent OTA reaches you automatically on next launch — no more manual reinstalls until a future runtime change. If you're not sure which version you have, open Configurações → Sobre and check; if it says anything older than `0.19.4`, please reinstall using the link above. |
| **OTA channel** | `preview`, mapped to `runtimeVersion: "1"` (hand-managed, decoupled from app version). Future patch releases (0.19.5, 0.19.6, …) on the same runtime arrive as silent OTAs — no APK reinstall needed. A fresh APK is only required when the runtime version itself changes (native module additions or Expo SDK upgrade). |
| **What's in 0.19.4** | (1) **Build/OTA fix** — APK now embeds the `preview` EAS Update channel, so future JS-only fixes will actually reach you. (2) Bundles all of the **0.19.3 i18n leak fixes** that didn't reach you previously: Home tab labels (eyebrow / Income / Expenses / Net / Accounts), bottom-nav titles (Home / Ledger), Save & Delete buttons on the new/edit transaction screens, the Home date label, plus Settings sibling leaks (Add/Edit account & category screens, archive/unarchive labels, validation errors) all now respect the language setting instead of falling back to Portuguese. (3) Bundles all of the **0.19.2 Insights fixes** (per-bar / per-point outlier fallback). (4) Bundles the **0.19.1 Insights freeze fix** and the €99,999,999.99 amount cap. **Default category names are NOT affected** — they are seeded into the on-device database at first launch in the language you picked then, and stay that way (changing the language afterwards re-translates UI strings, not stored data). See `docs/app-behavior.md` §3.1, §5.3, §5.5 and the *Tester note* boxes for what to verify. |
